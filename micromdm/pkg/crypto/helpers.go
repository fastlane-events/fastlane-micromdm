...
// Signing time validity errors are ignored to allow devices with expired
// SCEP certificates to continue communicating until they can re-enroll.
func (v *PKCS7Verifier) Verify(p7 *pkcs7.PKCS7) error {
	...
	if err != nil && strings.Contains(err.Error(), "is outside of certificate validity") {
		// Try with negative skew first
		err2 := p7.VerifyWithChainAtTime(nil, time.Now().Add(-v.MaxSkew))
		if err2 != nil && strings.Contains(err2.Error(), "is outside of certificate validity") {
			// Expired signing cert — allow it through so the device can still
			// receive commands (e.g. EraseDevice) and eventually re-enroll.
			fmt.Printf("level=warn msg=\"allowing expired device signing certificate\" err=%q\n", err.Error())
			return nil
		}
		return err2
	}

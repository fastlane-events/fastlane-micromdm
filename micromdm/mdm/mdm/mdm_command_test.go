...
func TestEndToEnd(t *testing.T) {
	var tests = []struct {
		...
		{
			name: "InstallApplication_ManagedConfiguration",
			requestBytes: []byte(
				`{"udid":"BC5E2DA4-7FB6-5E70-9928-4981680DAFBF","request_type":"InstallApplication","identifier":"events.fastlane.workstation","management_flags":1,"change_management_state":"Managed","configuration":{"fl_device_id":"device-abc-123"}}`,
			),
			testFn: func(t *testing.T, parts endToEndParts) {
				// json unmarshal carries the managed app configuration dictionary
				if have := parts.req.Command.InstallApplication.Configuration["fl_device_id"]; have != "device-abc-123" {
					t.Errorf("fl_device_id from json is %q, want %q", have, "device-abc-123")
				}
				// the configuration must survive the proto round-trip used by the command queue
				if have := parts.fromProto.Command.InstallApplication.Configuration["fl_device_id"]; have != "device-abc-123" {
					t.Errorf("fl_device_id after proto round-trip is %q, want %q", have, "device-abc-123")
				}
				// the device-facing plist must contain a Configuration dict with the value,
				// which Apple delivers as com.apple.configuration.managed
				needToSee := [][]byte{
					[]byte(`Configuration`),
					[]byte(`fl_device_id`),
					[]byte(`device-abc-123`),
					[]byte(`events.fastlane.workstation`),
					[]byte(`ChangeManagementState`),
					[]byte(`Managed`),
				}
